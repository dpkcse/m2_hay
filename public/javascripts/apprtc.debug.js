...
var AppController = function(loadingParams) {
  ...
  Promise.resolve(paramsPromise).then(function(newParams) {
    ...
    if(getCookie('call_status') =='reload'){
        ...
        // call_msg_send=true;
        // console.log('call_msg_send',call_msg_send);
        window.location.replace('/hayven/chat/'+'personal'+'/'+ hangup_id +'/'+ encodeURI(conversation_id) +'/'+ encodeURI(hangup_name) +'/' + encodeURI(hangup_img));
      //}
    }
    //else{

        if(is_chrome){
          alert('Sorry, chrome browser not supported!');
          var hangup_id=0;
          ...
          if(user_id == caller_id){
            ...
            var call_status = document.getElementById('call_status').innerHTML;
              console.log('call_ hangup_before chrome');
              // caller hang up for chrome
          ...
          if(roomid !='false'){
            ...
            // mycalltimer = setTimeout(function(){
...
AppController.prototype.hangup_ = function() {
  console.log('AppController. prototype. hangup_');
...
AppController.prototype.transitionToActive_ = function() {
  ...
  this.displayStatus_("");
   ...
   if(media_status==true && getCookie('call_status') =='receive'){
     ...
     var h1 = document.getElementById('call_status'),
...
Call.prototype.hangup = function(async) {
  console.log('Call.prototype.hangup'+async+call_msg_send);
  if(async){
    setCookie('call_status','new',1);
  ...
  if(user_id == caller_id){
    ...
    var call_status = document.getElementById('call_status').innerHTML;
    // caller hang up before receive ..
    if(call_status=='CALLING'){
      console.log('call_ hangup_before');
      ...
      socket.emit('call_hangup_before', {
        ...
        reload_status: async,
        call_status: call_msg_send,
        call_duration: document.getElementById('call_status').innerHTML
    ...
    console.log('call_ hangup_after');
    // receiver hang up after receive ..
    socket.emit('call_hangup_after', {
      hangup_id : hangup_id,
      hangup_name:hangup_name,
      hangup_img:hangup_img,
...
Call.prototype.onUserMediaError_ = function(error) {
  ...
  if(user_id == caller_id){
    ...
    socket.emit('call_reject_media_caller', {
      caller_id: caller_id,
      partner_id: partner_id,
      partner_name:partner_name,
      partner_img:partner_img,
    ...
    socket.emit('call_reject_media_receiver', {
      caller_id: caller_id,
      partner_id: partner_id,
      partner_name:partner_name,
      partner_img:partner_img,
...
PeerConnectionClient.prototype.startAsCaller = function(offerOptions) {
  ...
  this.pc_.createOffer(constraints).then(this.setLocalSdpAndNotify_.bind(this)).catch(this.onError_.bind(this, "createOffer"));
    ...
    if(getCookie('call_status') !='receive' && media_status !=false){
      if(call_type=='audio'){

        socket.emit('audiocall_req', {

          my_id : user_id, // caller_id
          to_id : partner_id, // partner_id
          my_img: user_img,
          to_img: partner_img,
